                    ...
                    <div class="col">
                        <div class="card shadow">
                            <div class="card-header py-3">
                                <p class="text-primary m-0 font-weight-bold">Edit Transaksi</p>
                            </div>
                            <div class="card-body">
                                <?php
                                ...
                                include "koneksi.php";
                                $id = $_GET['id'];
                                $query = mysqli_query($connect, "SELECT * FROM transaksi where id_order='$id' ") or die(mysqli_error($connect));
                                ...
                                while ($tr = mysqli_fetch_array($query)) {
                                    ...
                                    $k = mysqli_query($connect, "SELECT * FROM penyewa where no_ktp='$no_ktp' ") or die(mysqli_error($connect));
                                    while ($kt = mysqli_fetch_array($k)) {
                                        $nama = $kt['nama'];
                                    }
                                }
                                ?>
                                <form method="post" action="aksi_edit_transaksi.php?id=<?php echo    $id;    ?>">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="no_ktp" name='no_ktp'>
                                            ...
                                            $queryktp = "SELECT no_ktp FROM transaksi where id_order= $id";
                                            $result    = mysqli_query($connect, $query) or die(mysqli_error($connect));
                                            $resultktp = mysqli_query($connect, $queryktp) or die(mysqli_error($connect));
                                            $rowktp    = mysqli_fetch_array($resultktp);
                                            while ($row    = mysqli_fetch_array($result)) {
                                                ?>
                                                <option value="<?php echo $row['no_ktp']; ?>" <?php if ($row['no_ktp'] == $rowktp['no_ktp']) echo 'selected = "selected"'; ?>><?php echo $row['no_ktp']; ?></option>
                                            <?php } ?>
                                        </select></div>
                                    <div class="form-group">
                                        <div class="form-group">
                                            ...
                                            <input value="<?php echo $nama; ?>" class="form-control" type="text" id="nama" name="nama">
                                        </div>
                                        <div class="form-group">
                                            <label for="address">
                                                <strong>Alat</strong>
                                            ...
                                            <select class="form-control" name='alat'>
                                                ...
                                                $queryalat = "SELECT id_alat FROM transaksi where id_order= $id";
                                                $result    = mysqli_query($connect, $query) or die(mysqli_error($connect));
                                                $resultalat = mysqli_query($connect, $queryalat) or die(mysqli_error($connect));
                                                $rowalat    = mysqli_fetch_array($resultalat);
                                                while ($row    = mysqli_fetch_array($result)) {
                                                    ?>
                                                    <option value="<?php echo $row['id_alat']; ?>" <?php if ($row['id_alat'] == $rowalat['id_alat']) echo 'selected = "selected"'; ?>><?php echo $row['nama']; ?></option>
                                                <?php } ?>
                                            </select></div>
                                        <div class="form-group">
                                            <label for="address">
                                            ...
                                            <input value="<?php echo $jml; ?>" class="form-control" type="number" min="1" name="jml" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="address">
                                            ...
                                            <input value="<?php echo $tgl_sewa; ?>" type="date" class="form-control" name="tgl_sewa">
                                        </div>
                                        <div class="form-group">
                                            <label for="address">
